...
static void split_shared_pmd(void)
{
	...
	if (!check_huge_anon(one_page, nr_pmds * pmd_pagesize, nr_pmds, pmd_pagesize))
		ksft_exit_fail_msg("No THP is allocated\n");
	...
	if (pid == -1) {
		...
		if (WIFEXITED(status))
			...
			ksft_print_msg("Child terminated by signal: %d\n", WTERMSIG(status));
		else
			ksft_print_msg("Child stopped or other state\n");
	}
