...
static bool node_random_pgdat_free_space_enough(struct pglist_data *pgdat,
						int demote_pages)
{
	int z;
	...
	for (z = pgdat->nr_zones - 1; z >= 0; z--) {
		...
		if (zone_watermark_ok(zone, 0,
				      high_wmark_pages(zone) + demote_pages,
				      ZONE_MOVABLE, 0))
			return true;
	}
				      ...
				      struct random_migrate_state *rd_state)
{
	...
	if (node_random_pgdat_free_space_enough(pgdat, rd_state->nr_page)) {
